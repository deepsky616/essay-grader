...
**설계 참조:** 2절, 4절, 7.1절, 7.7절, 8절, 10절, 11절, 12절. 교사가 붙여넣은 명렬표를 지역 데이터베이스의 학급과 학생으로 만들고, 결시자는 기대 학생 수 계산에서 제외할 수 있게 한다.

> **구현 계약 보강:** 학급명과 학생 이름을 앞뒤 공백 제거 뒤 길이 검사하고 번호는 엄격한 양의 정수로 제한한다. 학급당 500명과 붙여넣기 본문 길이를 제한하며, 중복 번호를 붙여넣기와 생성 양쪽에서 막는다. 읽지 못한 줄 오류에는 민감한 원문을 되돌려 주지 않는다. 학생 수정은 경로의 학급에 실제로 속한 학생에게만 허용하고 빈 수정은 거절한다.
...
**설계 참조:** 2절, 4절, 5절, 7.1절부터 7.7절, 8절, 10절, 11절, 12절, 14절. 빈 답안지 원본을 200 dpi의 공통 좌표계로 보여 주고 응답 및 식별정보 영역을 저장한 뒤, 마커와 응답 박스가 들어간 배부용 PDF를 만든다.

> **구현 계약 보강:** 원본과 생성 PDF는 안전한 업로드 폴더 바로 아래의 일반 파일만 허용하고 지역 경로를 응답에 노출하지 않는다. 실제 PDF 쪽 수와 렌더 크기를 다시 검사하며, 영역은 엄격한 정수 좌표와 페이지 안쪽 배치, 문항 유일성, 식별정보 한 개와 겹침 금지를 지켜야 한다. 새 원본을 선택하면 기존 영역과 인쇄 준비 상태를 무효화하고 같은 원본 재호출은 유지한다. 생성 PDF는 고유 이름과 600 권한으로 저장하고 비공개 캐시 지시와 고정 내려받기 이름으로 제공한다.
...
**설계 참조:** 2절, 4절, 5절, 7.1절부터 7.7절, 8절, 10절, 11절, 12절, 14절. 안전하게 저장한 스캔 PDF를 독립 세션의 뒤 작업으로 처리하고, 전체 검증을 통과한 학생 및 문항 결과만 지역 파일과 데이터베이스에 함께 반영한다.

> **구현 계약 보강:** 업로드 전에 평가, 학급, 응시 학생, 템플릿, 응답 및 식별정보 영역을 확인하고 PDF 크기와 실제 쪽 수를 검사한다. 결과는 학생 수와 순서, 페이지와 문항 이미지, 품질과 배정 상태를 모두 검사한 뒤 700 임시 폴더의 600 PNG로 완성한다. 임시 폴더를 최종 위치로 바꾸고 데이터베이스를 커밋하며 어느 쪽이 실패해도 새 레코드와 결과 폴더를 정리한다. 작업 예약 전 실패는 배치와 PDF를 함께 되돌리되 예약 뒤 연결 저장 실패는 실행 중 작업의 PDF를 지우지 않는다. 오류 원문과 지역 경로는 공개 상태에 넣지 않는다.
...
**설계 참조:** 7.7절의 외부 전송 경계와 4번 실명 노출 감지. 외부 전송 직전에 현재 명렬표의 이름을 검사하되, 이름 집합과 발견한 실제 이름은 외부 요청이나 감사 기록에 남기지 않는다.

> **구현 계약 보강:** 용어 공급자는 호출할 때마다 새 데이터베이스 세션을 열어 모든 학급과 결시자의 최신 이름을 읽고 반드시 닫는다. 앞뒤 공백을 없앤 두 글자 이상의 이름만 사용하며, 조회 실패를 빈 집합으로 바꾸지 않고 전송 관문까지 전파해 제공자 호출을 차단한다. 루브릭 생성 경로는 요청 세션의 바인드로 독립 세션 팩토리를 만들어 공급자에 넘긴다.
...
**설계 참조:** 7.7절 1번부터 3번의 식별정보 영역 지정, 지역 처리, 전송 전 배제와 14절의 템플릿 영역 지정 화면. 응답 영역과 식별정보 영역은 겹치지 않아야 하며, 배부용 답안지를 만들기 전에 두 종류가 모두 있어야 한다.

> **구현 계약 보강:** 마우스와 터치를 같은 포인터 입력으로 처리하고 화면 표시 크기를 원본 화소 좌표로 바꾸되 좌표를 이미지 경계에 고정한다. 화면에서 중복 문항 번호, 둘째 식별정보 영역, 두 종류 영역의 겹침과 너무 작은 영역을 먼저 거절하며 서버가 저장과 배부용 파일 생성 때 다시 검사한다. 식별정보 영역이 없으면 직접 API 요청도 배부용 파일을 만들 수 없다. 쪽별 영역 목록과 개별 지우기, 저장하지 않은 변경 표시, 자동 새 창이 막혀도 쓸 수 있는 내려받기 링크를 제공한다.
...
**설계 참조:** 2절, 4절, 5절, 7.1절부터 7.7절, 8절, 10절, 11절, 12절, 14절. 명렬표는 지역 저장소에만 두고 결시자를 뺀 응시 인원으로 배치 기대값을 정하며, 배치 실패와 학생 배정 검토 상태를 교사가 한 화면에서 확인하고 바로잡을 수 있게 한다.

> **구현 계약 보강:** 화면은 100MB를 넘는 파일과 잘못된 평가 번호를 먼저 거절한다. 올리기가 시작되면 그 배치의 학급을 고정하며, 상태 폴링은 앞 요청이 끝난 뒤 다음 요청을 보내고 일시 오류 뒤에도 계속된다. 이미 다른 제출에 배정된 학생을 고르면 유일성 제약을 깨는 덮어쓰기 대신 같은 배치 안의 두 학생 배정을 원자적으로 맞바꾼다.
...
**설계 참조:** 2절, 7.1절부터 7.7절, 11절, 12절, 14절. 실제 출력과 300dpi 스캔으로 마커 정합, 필기 추출, 식별정보 지역 처리, 급지 사고 전체 중단을 현장 조건에서 확인한다.
