...
이름보다 하나 빠르다. 따라서 이 계획의 P4는 설계 문서 12절 표의 P5 완료 기준인
교사 검토와 확정을 구현한다.

| 작업 | 함께 구현하는 설계 절 | 구현 계약 |
|---|---|---|
| 1. 수정 이력 모델 | 8절, 10절 | 이전 점수, 새 점수, 출처, 고정 지역 교사 주체와 시각을 남긴다. |
| 2. 확정 로직 | 8절, 10절 | 수락과 수정 모두 같은 경로로 확정하고 이력을 빠뜨리지 않는다. |
| 3. 실측 일치율 | 8절, 13절 | 실제 수정 이력이 있는 확정만 표본으로 세며 빈 표본을 안전하다고 보지 않는다. |
| 4. 검토 API | 7.7절, 8절, 10절 | 성공한 실행과 확정 루브릭만 받고 지역 저장소의 일반 이미지 파일만 제공한다. |
| 5. 검토 화면 | 8절, 12절 | 문항 단위 이동, 점수 단축키, 크롭과 실제 전체 페이지 전환, 일괄 확정을 제공한다. |
| 6. 일치율 보고서 | 8절, 13절 | 문항과 유형별 교사 일치율, 자동 경로 수정 건수, 학생별 현재 총점을 구분해 보인다. |
| 7. 검토 속도 측정 | 13절, 14절 | 실제 답안과 교사 종이 채점 기준 시간이 있을 때만 현장 수치를 기록한다. |
